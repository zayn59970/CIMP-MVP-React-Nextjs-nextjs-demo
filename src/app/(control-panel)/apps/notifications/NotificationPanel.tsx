...
function NotificationPanel() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      const { data } = await useGetUndismissedNotificationsQuery(userId);
...
//       title: "New CIMP version is released! ",
	  ...
	  enqueueSnackbar(first.title, {
		...
		content: (
		  <NotificationTemplate
			item={first}
			onClose={() => {
			  handleDismiss(first.id);
			  closeSnackbar(first.id);
			}}
		  />
		)
	  });
