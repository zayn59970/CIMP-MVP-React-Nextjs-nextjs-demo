...
function BoardCardForm() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [boardResponse, membersResponse, labelsResponse, listsResponse] = await Promise.all([
        supabaseClient.from('boards').select('*').eq('id', boardId).single(),
        supabaseClient.from('members').select('*'),
        supabaseClient.from('labels').select('*').eq('board_id', boardId),
        supabaseClient.from('lists').select('*').eq('board_id', boardId),
      ]);
...
const updateCardData = useDebounce(async (newCard: ScrumboardCard) => {
    try {
      const { error } = await supabaseClient.from('cards').update(newCard).eq('id', newCard.id);
      if (!error) {
        dispatch(
          showMessage({
            message: 'Card Saved',
  ...
  console.log("dkmflsdnmvlksdnka",board);
	return (
